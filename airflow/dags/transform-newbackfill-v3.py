...
def get_raw_file(logical_date_string, **kwargs):
    ...
    logging.info(f"Transforming position for {year}-{month}-{day} {hour}:{minute}...")
...
# Criando o DAG
with DAG(
    "transform-newbackfill-v3",
    default_args=default_args,
    description="Load raw data from MinIO, process it, and store it in PG",
    start_date=datetime(2026, 1, 13, 18, 42),
    end_date=datetime(2026, 1, 14, 2, 58),
